...
# Define boundaries for the greater NYC area
lat_min, lat_max = 40.4774, 40.9176
lon_min, lon_max = -74.2591, -73.7004

# Filter data to include only points within the boundaries
data = data[(data['Squirrel Latitude (DD.DDDDDD)'] >= lat_min) &
            (data['Squirrel Latitude (DD.DDDDDD)'] <= lat_max) &
            (data['Squirrel Longitude (-DD.DDDDDD)'] >= lon_min) &
            (data['Squirrel Longitude (-DD.DDDDDD)'] <= lon_max)]
